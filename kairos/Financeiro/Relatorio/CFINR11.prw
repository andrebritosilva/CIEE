...
ElseIf mv_par01==3
	If mv_par06==1
		Tit := OemToAnsi("Analitico de ")+DTOC(mv_par02) + " a " +Dtoc(mv_par03)+" por Data e Competência"  // por Data e Beneficiario
	Else
		Tit := OemToAnsi("Sintetico de ")+DTOC(mv_par02) + " a " +Dtoc(mv_par03)+" por Data e Competência"  // por Data e Beneficiario
	EndIf
ElseIf mv_par01==4
	If mv_par06==1
...
//FINR11AGL(_cArqtrab) //Foi retiado por gerar impacto na impressão do relatório.
...
While !Eof()
	...
	While !Eof() .And. _cE5_ORDEM == E5_ORDEM
		...
		If (aRecon[1][ENTRADA] <> 0 .Or. aRecon[1][SAIDA] <> 0)
			...
			If Empty(_cDescNat)			// se não encontrou na ordem 1 procura na ordem 4
...
Rotina para verificação de registro
@author  	Totvs
@since     	01/01/2015
@version  	P.11.8      
@return   	Nenhum 
...
Rotina para verificação da natureza 2013
@author  	Totvs
@since     	01/01/2015
@version  	P.11.8      
@return   	Nenhum 
