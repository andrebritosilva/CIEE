...
If cAlias == "SE1" 
	aLegenda := { {"BR_VERDE"		, "Titulo em aberto" },;	 
					...
					{"BR_AMARELO"	, "Titulo Serasa"},;
					{"BR_LARANJA"	, "Titulo Global"}} 	
	...
	If Empty(lVez)   // Via menu, a opcao passada eh 1.
		...
		Aadd(uRetorno, { 'ROUND(E1_SALDO,2) == ROUND(E1_VALOR,2) .and. E1_SITUACA == "F"'			, aLegenda[Len(aLegenda)][1]	} ) //"Titulo Serasa"
		Aadd(uRetorno, { 'ROUND(E1_SALDO,2) == ROUND(E1_VALOR,2) .and. E1_SITUACA == "Z"'			, aLegenda[Len(aLegenda)][1]	} ) //"Titulo Global"
...
ELSEIF cAlias == "SE2" .AND. !ISINCALLSTACK("FINA426")
	...
	aLegenda := { {"BR_AMARELO" , "Aguardando Liberacao" },;
					...
					{"BR_CANCEL"	, "Titulo Reprovado pelo WF do Bordero"},;
					{"BR_VIOLETA" , "Adiantamento em aberto" },;
					{"BR_BRANCO"  , "Adiantamento com saldo"}}
...
//		Aadd(uRetorno, { ' !( SE2->E2_TIPO $ MVPAGANT ).and. EMPTY(E2_DATALIB) .AND. (SE2->E2_SALDO+SE2->E2_SDACRES-SE2->E2_SDDECRE) > GetMV("MV_VLMINPG") .AND. E2_SALDO > 0', aLegenda[1][1] } )
		Aadd(uRetorno, { ' EMPTY(E2_DATALIB) .AND. (SE2->E2_SALDO+SE2->E2_SDACRES-SE2->E2_SDDECRE) > GetMV("MV_VLMINPG") .AND. E2_SALDO > 0', aLegenda[1][1] } )
