...
/*/{Protheus.doc} CJOBK15
Rotina de calculo da bolsa auxílio na tela de liberação de inconsistência
@author carlos.henrique
@since 06/11/2019
@version undefined
@type User function
/*/
User Function CJOBK15(dDatInc)
Local _lJob		:= GetRemoteType() == -1 // Verifica se é job
Local lProSPBA  := .F.
Local dGetRef   := dDataBase
Local dBkpDta	:= dDataBase
default dDatInc := dGetRef

Begin Sequence

	If  IsInCallStack("U_FFIN1003")
		dDataBase:= dDatInc
		lProSPBA  := .T.
	else
		If _lJob 
			dDataBase:= MV_PAR01  //Data do parametro do SCHEDULE
			U_CJBK15LOG(1,"Iniciando processamento via schedule.")
			U_CJBK15LOG(1,"Empresa:" + CEMPANT )
			U_CJBK15LOG(1,"Filial :" + CFILANT )
			U_CJBK15LOG(1,"Data   :" + DTOC(dDataBase) )
			lProSPBA  := .T.
		else
			DEFINE MSDIALOG oDlg TITLE "Calculo bolsa auxílio - inconsistências" From 000,000 to 085,280 COLORS 0, 16777215 PIXEL
	...
	if lProSPBA
		...
		If !_lJob
			...
			//processa calculo
			FWMsgRun(,{|| CJOBK15PRC() },,"Processando calculo da bolsa auxílio - inconsistências, aguarde...")
...
U_CJBK15LOG(1,"Inicio Processamento")

BeginSql Alias _cAliasFol
	SELECT ZCV.R_E_C_N_O_ NREGZCV
	FROM %TABLE:ZCV% ZCV
	WHERE ZCV.ZCV_FILIAL= %xfilial:ZCV%
	AND ZCV.ZCV_STATUS IN ('2','3')
	AND ZCV.ZCV_DATPGT=%Exp:_dDataRef%
	AND ZCV.%notDel%	
	ORDER BY ZCV.R_E_C_N_O_
EndSql
//GETLastQuery()[2]
While (_cAliasFol)->(!Eof())

	AADD(_aRecFol,(_cAliasFol)->NREGZCV)
...
IF !EMPTY(_aRecFol)
    ...
    IF cGrid == "1" 
        ...
        lRet := oGrid:BatchExec("U_CJBK15AMB",{cEmpAnt,cFilAnt,""},"U_CJBK15CAL",_aRecFol)

        If !lRet .and. Empty(oGrid:aGridThreads)
			U_CJBK15LOG(1,"Nenhum Agente do GRID disponivel no Momento.")
        EndIf
		...
		For nX:= 1 to len(_aRecFol)
			STARTJOB("U_CJBK15AMB" ,GetEnvServer(),.T.,{cEmpAnt,cFilAnt,"U_CJBK15CAL("+CVALTOCHAR(_aRecFol[nX])+")"})
			//U_CJBK15AMB({cEmpAnt,cFilAnt,"U_CJBK15CAL("+CVALTOCHAR(_aRecFol[nX])+")"}) //DEBUGAR
		Next	
	endif		
else
	U_CJBK15LOG(1,"Nenhuma bolsa para calcular.")
endif

U_CJBK15LOG(1,"Fim do Processamento")

Return Nil

/*/{Protheus.doc} CJBK15AMB
...
@type function
/*/
USER Function CJBK15AMB(aParms)
...
U_CJBK15LOG(1,"Inciando calculo da bolsa auxílio")
...
@type function
/*/
User Function CJBK15CAL(nRecno)
...
DbSelectarea("ZC7")
ZC7->(DbSetOrder(1))
ZC7->(DbSeek(ZCV->ZCV_IDFOL))

IF ZCV->(!EOF()) .AND. ZC7->(!EOF())
	If SoftLock("ZCV") .and.  SoftLock("ZC7")	

		U_CJBK15LOG(1,"Calculando bolsa auxílio: " + ZC7->ZC7_IDFOL)

		_dDataRef  	:= ZCV->ZCV_DATPGT	
		dDataBase	:= _dDataRef
		...
		Begin transaction
			...
			Begin Sequence
				...
				IF EMPTY(_aProcesso := CJBK15RCJ(_cRoteiro,_cPeriodoMA))
					_cIniLog+= "A configuração de folha " + ALLTRIM(ZC7->ZC7_IDCFGF) + " não existe ou não possui os locais vinculados" + CRLF		
				ENDIF				

				if !EMPTY(_cIniLog)
					U_CJBK15LOG(2,"Falta configuração para inicio do processamento","3",_cIniLog)
					Break
				endif	
				...
				For _nPos := 1 To Len(_aProcesso)
					...
					IF !RCJ->(dbSeek(xFilial("RCJ")+ _aProcesso[_nPos][1] ))
						U_CJBK15LOG(2,"Processo "+ _aProcesso[_nPos][1] +" não localizado","3")
						Break		
					ENDIF				

					_cNumPag:= CJBK15NPG(_cPeriodoMA,RCJ->RCJ_CODIGO,.F.)	

					//Tratamento para geração de períodos da folha
					IF !CJBK15PER(_cRoteiro,RCJ->RCJ_CODIGO,_dDataRef,_cNumPag)
						U_CJBK15LOG(2,"Período "+ANOMES(_dDataRef)+" não localizado na tabela RCH","3",;
					...
					BeginSql Alias _cAliasTrb
						...
						INNER JOIN %TABLE:SRA% SRA ON
							...
							AND SRA.RA_MAT		= %Exp:ZCV->ZCV_MAT% 
							AND SRA.RA_PROCES   = %Exp:_aProcesso[_nPos][1]% 
							AND SRA.RA_XATIVO   = 'S' 
							AND SRA.RA_XVALIBC  = '2'
							AND SRA.%notDel%
					...
					While (_cAliasTrb)->(!Eof())
						...
						If (_cAliasTrb)->NREGSRV == 0
							U_CJBK15LOG(2,"Tipo Kairós não relacionado a uma verba SRV.","3",;
						...
						If (_cAliasTrb)->NREGSRA == 0
							U_CJBK15LOG(2,"Id não relacionado na tabela SRA.","3",;
					...
					//Fecha o período
					_cNumPag:= CJBK15NPG(_cPeriodoMA,RCJ->RCJ_CODIGO,.T.)
					...
					U_CJBK15LOG(1,"Fechando bolsa auxílio: " + ZC7->ZC7_IDFOL)
					...
					IF TCSQLEXEC("DELETE "+RETSQLNAME("RGB")+" WHERE RGB_XIDFOL='"+ZC7->ZC7_IDFOL+"' AND D_E_L_E_T_=''"+;
						...
						U_CJBK15LOG(1,TCSQLERROR())
					ENDIF			
					
					//Exclui o novo periodo
					CJBK15DPE(_cRoteiro,RCJ->RCJ_CODIGO,_dDataRef,_cNumPag)

					U_CJBK15LOG(2,"Processamento finalizado com sucesso","4","")				
					
				Next
			
			Recover

				_cBkpMLog  := ZCV->ZCV_MSGLOG
				_cBkpLCom  := ZCV->ZCV_LOGCOM			
				...
				IF !EMPTY(cError)
					...
					U_CJBK15LOG(2,"Falha no calculo","3",cError)
				
				ELSEIF EMPTY(cError) .AND. !EMPTY(_cBkpMLog)

					U_CJBK15LOG(2,_cBkpMLog,"3",_cBkpLCom)					
	...
	ELSE
		U_CJBK15LOG(1,"Não foi possivel travar o registro da folha: " + ZC7->ZC7_IDFOL)	
	ENDIF
ELSE
	U_CJBK15LOG(1,"Não foi possivel posicionar o recno: " + CVALTOCHAR(nRecno) + " da tabela ZCV.")	
ENDIF

Return
/*/{Protheus.doc} CJBK15LOG
...
@type function
/*/
User Function CJBK15LOG(nTpLog,cMsgLog,cStatus,cLogCom)
default cStatus:= ""
default cLogCom:= ""
...
Case nTpLog == 1 //Exibe log em tela ou console
	...
	CONOUT("["+LEFT(DTOC(Date()),5)+"]["+LEFT(Time(),5)+"][CJOBK15] " + cMsgLog)

CASE nTpLog == 2 //Grava log na tabela ZCV posicionada

	DbSelectarea("ZCV")
	ZCV->(DbGoto(nRecZCV))

	RECLOCK("ZCV",.F.)
	ZCV->ZCV_MSGLOG:= cMsgLog
	ZCV->ZCV_STATUS:= cStatus
	ZCV->ZCV_LOGCOM:= cLogCom	
	MSUNLOCK()
...
Local aParam := {"P","CJOBK15","",{},""}    
Return aParam
/*/{Protheus.doc} CJBK15PER
...
@type function
/*/
Static function CJBK15PER(_cRoteiro,_cProcesso,_dDataRef,_cNumPag)
...
@type function
/*/
Static function CJBK15DPE(_cRoteiro,_cProcesso,_dDataRef,_cNumPag)
...
@type function
/*/
static function CJBK15NPG(_cPeriodoMA, _cProcesso, _lFecha)
Local _cAliasRCH := GetNextAlias()
...
@type function
/*/
static function CJBK15RCJ(_cRoteiro,_cPeriodoMA)
